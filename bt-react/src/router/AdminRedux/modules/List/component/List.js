    ...
    <tbody>
      {
        questions.map((ques, index) => (
          ...
          key={`item-${ques.id}`}
           ...
           index={index} ques={ques} remove={remove} toEdit={toEdit} />
        ))
      }
    </tbody>
  );
...
export default function List({ questions, remove, setQuestions }) {
  ...
  const toCreate = () => {
    history.push('/admin-redux/create')
  }
  const toEdit = useCallback((id)=>{
    history.push(`/admin-redux/${id}/edit`)
  ...
  return (
    <div className='container'>
      ...
      <table className="table">
        ...
        <SortableList questions={questions} remove={remove} onSortEnd={onSortEnd} toEdit={toEdit} />
      </table>
    </div>
  )
}
